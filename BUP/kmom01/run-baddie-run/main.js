  ...
  document.onkeydown = function(event) {
    ...
    switch(key) {
      ...
      case 32:  // space = jump
        ...
        setTimeout(function(){moveIt(0, step);console.log('Timer!');}, 500);
        break;
      case 82:  // r = random
        moveIt(step*Math.floor(Math.random()*(3)-1), step*Math.floor(Math.random()*(3)-1));
        break;
      case 83:  // s = salto
        if (target.className == "baddie right") {
          console.log('Right SALTO!')
          moveIt(step, -step);
          setTimeout(function(){target.className='baddie rightdoublesalto';},50); //do salto on way up
          setTimeout(function(){moveIt(step, step);}, 500); // back down after salto      
          setTimeout(function(){target.className='baddie right';},1000); //face forward
        } else if (target.className == "baddie left"){
          console.log('Left SALTO!')
          moveIt(-step, -step);
          setTimeout(function(){target.className='baddie leftdoublesalto';},50); //do salto on way up
          setTimeout(function(){moveIt(-step, step);}, 500); // back down after salto      
          setTimeout(function(){target.className='baddie left';},1000); //face forward
        }
